...
APACHE_CERT_DIR="$APACHE_KEYTAB_DIR/tls"
HTTP_PRINCIPAL="HTTP/web.fis.epn.ec@FIS.EPN.EC"
...
require_keytab_principal "$APACHE_KEYTAB_DIR/web.keytab" "$HTTP_PRINCIPAL"
...
if ! ss -ltnH 'sport = :443' | grep -q .; then
    print_error "Apache no esta escuchando en el puerto TCP 443"
    exit 1
fi
